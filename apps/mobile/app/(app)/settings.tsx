...
export default function SettingsScreen() {
  ...
  useEffect(()=>{
    supabase.auth.getUser().then(async({data:{user:u}})=>{
      ...
      const{data:m}=await supabase.from("org_members").select("org_id,role,organizations(id,name,ftc_team_number)").eq("user_id",u.id).single();
      if(!m) return;
      const o=m.organizations as {id:string;name:string;ftc_team_number:string|null};
      ...
      setMembers((mems??[]) as typeof members);
    });
  },[]);
