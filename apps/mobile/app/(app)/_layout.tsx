  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data: m } = await supabase
        ...
        .maybeSingle();
      if (!m) {
        // Check for a pending org from signup (created after email confirmation)
        ...
        if (pending) {
          ...
          if (userId === user.id) {
            ...
            const { data: org } = await supabase.from("organizations").insert({ name, ftc_team_number: teamNumber, slug, created_by: user.id }).select("id").single();
            if (org) {
