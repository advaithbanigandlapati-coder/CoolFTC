...
export default function AnalyticsPage() {
  ...
  // Infer season from event key (e.g. "2025-DECODE-TEST" -> 2025)
  useEffect(() => {
        ...
        {heatPts.length > 0 ? (
          <div className="flex gap-6 flex-wrap">
            <div className="relative rounded overflow-hidden border border-white/[0.065]" style={{ width: HEATMAP_W, height: HEATMAP_H, background: "#0F0F17" }}>
              <div className="absolute top-2 left-2 font-mono text-[9px] text-white/20">CLOSE</div>
              <div className="absolute top-2 right-2 font-mono text-[9px] text-white/20">FAR</div>
            ...
            <div className="font-mono text-xs text-white/40 max-w-xs">
              <div className="mb-2">Total data points: <span className="text-white/80">{heatPts.length}</span></div>
              <div>Heatmap intensity shows where this team has scored across all their matches at this event.</div>
            </div>
          </div>
        ) : (
          <div className="bg-white/[0.02] border border-dashed border-white/10 rounded-lg p-8 text-center">
            <div className="font-mono text-[10px] text-white/40 tracking-widest mb-2">WAITING FOR DATA</div>
            <p className="text-sm text-white/60 max-w-md mx-auto">
              This heatmap fills in automatically as scouts tap field positions during match scouting.
              No positional data has been recorded yet for Team {team || "—"} at {eventKey}.
            </p>
          </div>
        )}
      </div>}
