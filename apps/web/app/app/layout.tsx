...
const NAV = [
  ...
  { href: "/app/mobile",     icon: "◰", label: "Mobile App" },
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      // Handle pending org (signed up with email confirmation, completing on first login)
      const pending = localStorage.getItem("pendingOrg");
      if (pending) {
        try {
          const { name, teamNumber, userId } = JSON.parse(pending) as { name: string; teamNumber: string | null; userId: string };
          if (userId === data.user.id) {
            ...
            if (!existing) {
              ...
              const { data: org } = await supabase.from("organizations").insert({
                name, ftc_team_number: teamNumber, slug, created_by: data.user.id,
              }).select("id").single();
              if (org) await supabase.from("org_members").insert({ org_id: org.id, user_id: data.user.id, role: "admin" });
            }
            localStorage.removeItem("pendingOrg");
          }
        } catch { localStorage.removeItem("pendingOrg"); }
      }
    });
  }, [router]);
