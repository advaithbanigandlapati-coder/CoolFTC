    ...
    sb.auth.getUser().then(async ({ data: { user: u } }) => {
      ...
      if (!m) {
        ...
        if (pendingRaw) {
          try {
            ...
            if (pending.userId === u.id) {
              const r = await fetch("/api/org/create", {
                ...
                body: JSON.stringify({ name: pending.name, teamNumber: pending.teamNumber }),
              });
              const d = await r.json();
              if (r.ok && d.org) {
                localStorage.removeItem("pendingOrg");
                setOrg(d.org as OrgData);
          ...
          } catch { /* JSON parse failed or network error — fall through */ }
        }
        setNoOrg(true);
        setAuthLoaded(true);
        return;
  ...
  async function createOrg() {
    ...
    try {
      const r = await fetch("/api/org/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: newOrgName.trim(), teamNumber: newOrgTeam.trim() || null }),
      });
      const d = await r.json();
      if (!r.ok) {
        setCreateOrgErr(d.error ?? "Failed to create org. Try again.");
      ...
      setCreateOrgErr(e instanceof Error ? e.message : "Network error. Try again.");
    } finally {
