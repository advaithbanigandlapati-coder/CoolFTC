...
export async function POST(req: NextRequest) {
  ...
  const entryType: string = body.type ?? "match";

  // ── Pit scouting → scouting_entries table ────────────────────────────
  if (entryType === "pit") {
    const { type: _t, ...pitData } = body;
    const { data, error } = await db
      .from("scouting_entries")
      .upsert(
        { ...pitData, scouted_by: user.id, scouted_at: new Date().toISOString() },
  ...
  // ── Match scouting → match_scouting table (default) ──────────────────
  const { type: _t, ...matchData } = body;
  const { data, error } = await db
    .from("match_scouting")
    .upsert(
      { ...matchData, scouted_by: user.id, scouted_at: new Date().toISOString() },
