...
export type TeamSearchResult = {
  number: number; name: string; city: string | null; stateProv: string | null;
...
export async function searchTeams(query: string, limit = 25): Promise<TeamSearchResult[]> {
  ...
  try {
    ...
    const res = await fetch(url.toString(), {
      headers: { "User-Agent": "CoolFTC (team 30439)" },
      // @ts-ignore next-specific
      next: { revalidate: 60 },
    });
    if (!res.ok) return [];
    ...
    return (data ?? []).slice(0, limit).map(t => ({
      ...
      stateProv:  (t.stateProv as string | null) ?? null,
      country:    (t.country  as string | null) ?? null,
      rookieYear: (t.rookieYear as number | null) ?? null,
    }));
  } catch {
    return [];
  }
...
export async function searchEvents(query:string, season?:number, limit=15): Promise<EventSearchResult[]> {
  ...
  try {
    ...
    return filtered.slice(0, limit).map(e => ({
      ...
      // FTCScout REST API uses stateProv; normalize to state for our type
      state: ((e.stateProv ?? e.state) as string|null) ?? null,
